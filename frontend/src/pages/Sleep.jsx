...
import { Moon, Trash2, Plus, Clock, CalendarDays } from "lucide-react";
import {
  getSleepRecords,
  createSleepRecord,
  deleteSleepRecord,
...
function Sleep() {
  ...
  const loadSleepRecords = async () => {
    try {
      ...
      setSleepRecords(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error(err);
      setError(err.message || "Failed to load sleep records");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveSleep = async () => {
    if (!date || !sleepTime || !wakeTime) {
      setError("Please enter date, sleep time and wake time.");
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setError(err.message || "Failed to save sleep record");
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              Add your sleep information for today.
            </p>
          </div>
